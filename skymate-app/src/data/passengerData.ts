...
export function getAllSpecialRequests(): Array<{
  ...
  return Object.values(PASSENGER_DATABASE)
    .filter((passenger) => passenger.specialRequests && passenger.specialRequests.length > 0)
    .map((passenger) => ({
...
export function formatAllSpecialRequests(): string {
  ...
  const intro = `There are ${specialRequestsData.length} special request${specialRequestsData.length > 1 ? 's' : ''} for this flight: `;
  const requests = specialRequestsData.map((data) => {
...
export function formatAllPriorityMembers(): string {
  ...
  const intro = `There ${priorityMembers.length === 1 ? 'is' : 'are'} ${priorityMembers.length} priority member${priorityMembers.length > 1 ? 's' : ''} on this flight: `;
  const members = priorityMembers.map((member) => {
