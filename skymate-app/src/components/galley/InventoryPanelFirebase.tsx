...
function InventoryPanelFirebase() {
	...
	const handleMoveToQueue = async (itemId: string) => {
		try {
			await moveToQueue(itemId, quantityToMove);
	...
	const handleCompleteQueue = async (queueItemId: string) => {
		if (
			confirm(
				"Complete this item? It will be removed from inventory permanently."
			)
		) {
			try {
	...
	const handleReturnFromQueue = async (queueItemId: string) => {
		try {
			await returnFromQueue(queueItemId);
		} catch (error: any) {
			showError("Inventory Error", error.message);
		}
	};
		...
		<>
			<div className="h-full flex flex-col bg-gradient-to-br from-emerald-900 via-teal-900 to-cyan-900">
				...
				<div className="flex-1 overflow-hidden grid grid-cols-2 gap-4 p-6">
					...
					<div className="flex flex-col bg-emerald-800/50 backdrop-blur-lg rounded-xl shadow-sm border border-white/20 overflow-hidden">
												...
												return (
													<motion.div
														...
														className={`border-2 rounded-lg p-3 transition-all ${
															isSelected
																? "border-white bg-white/20 shadow-md text-white"
																: `bg-white/10 border-white/30 text-white hover:bg-white/20 hover:shadow-md cursor-pointer`
														}`}
														onClick={() =>
																...
																>
																	<div className="flex items-center gap-2">
																		...
																		<input
																			...
																			className="w-16 px-2 py-1 border border-white/30 bg-white/10 text-white rounded text-xs"
																			onClick={(e) => e.stopPropagation()}
																		/>
																		<button
																			onClick={(e) => {
																				e.stopPropagation();
																				handleMoveToQueue(item.id);
																			}}
																			className="flex-1 px-2 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition flex items-center justify-center gap-1 text-xs font-medium"
																		>
																			<ArrowRight size={14} />
																			Move
																		</button>
					...
					{/* QUEUE COLUMN */}
					<div className="flex flex-col bg-teal-800/50 backdrop-blur-lg rounded-xl shadow-sm border border-white/20 overflow-hidden">
						<div className="p-4 bg-gradient-to-r from-teal-700 to-cyan-700 text-white">
							<h3 className="text-lg font-semibold flex items-center gap-2">
								<PlayCircle size={20} />
								Queue (In Use)
							</h3>
						...
						<div className="flex-1 overflow-y-auto p-4 space-y-3">
							{queueItems.length === 0 ? (
								<div className="text-center py-12">
									...
									/>
									<p className="text-white/70">No items in queue</p>
									<p className="text-sm text-white/60 mt-1">
										Move items from main inventory to start
									</p>
								</div>
							) : (
								queueItems.map((item) => (
									<motion.div
										...
										exit={{ opacity: 0, x: -20 }}
										className="border-2 border-white/30 bg-white/10 rounded-lg p-4 text-white"
									>
										<div className="flex items-start justify-between mb-3">
											<div className="flex-1">
												...
												<p className="text-xs text-white/60 mt-1">
													Taken: {new Date(item.takenAt).toLocaleTimeString()}
												</p>
											</div>
											<span className="text-xs font-medium px-2 py-1 rounded bg-white/20 text-white">
												In Use
										...
										<div className="flex gap-2">
											<button
												onClick={() => handleReturnFromQueue(item.id)}
												className="flex-1 px-3 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition flex items-center justify-center gap-2 text-sm font-medium"
											...
											<button
												onClick={() => handleCompleteQueue(item.id)}
												className="flex-1 px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition flex items-center justify-center gap-2 text-sm font-medium"
												title="Complete and delete from inventory"
											>
												<Trash2 size={14} />
												Complete
											</button>
